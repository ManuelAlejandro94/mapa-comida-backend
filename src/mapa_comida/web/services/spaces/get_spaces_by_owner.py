...
def register_routes(app, scouts):
    ...
    @app.route('/spaces/space/owner/<id>', methods=['GET'])
    def get_spaces_by_owner(id):
        log_id = create_log_id()
        response = []
        app.logger.info(f'LOGID: {log_id} - ID de entrada: {id}')

        try:
            app.logger.info(f'LOGID: {log_id} - Búsqueda find_spaces_by_owner: {id}')
            results = scouts.find_spaces_by_owner(id)
            if results is None:
